...
my(@GFF, @list, @Ori);  # 添加@Ori变量声明
my(%list2, %gene, $ori, $n, $mRNA, %mRNA, $ORF, %ORF_list, %list);
...
foreach(@list){  
    if(/(STRG.+?)\s+(ORF\d+)\s+(\d+)\s+(\d+)\s+(.+?)\n/){ 
        ...
        #else{$ori="-"; $list{$1}="$4\t$3\t$ori\t$2";  $list2{"$1\t$2"}="$4\t$3\t$ori\t$2";} 
    } 
}
$n=keys%list2;  print "$n\n";

foreach(@GFF){
    if(/transcript.+?transcript_id\s+"(.+?)";.+?FPKM "(.+?)";/){  
        $transcript_id=$1;  
    }elsif(/exon.+?transcript_id\s+"($transcript_id)";/){
        $mRNA{$transcript_id}.=$_; 
    }
}
$n=keys%mRNA;  print "$n\n";

foreach(@GFF){
    if(/(chr_\d+|chr\w+)\s+\S+\s+transcript\s+(\d+)\s+(\d+)\s+\S+\s+(.)\s+.*?gene_id\s+"(.+?)";\s+transcript_id\s+"(.+?)";.*?FPKM "(.+?)";/){
        $chr=$1; $gene_start=$2; $gene_end=$3; $mRNA_id=$6; $gene_id=$5; $transcrpt_ori=$4;
        if(exists $list{$mRNA_id}){  # print "\n$mRNA_id\n";
            @exon=split/\n/,$mRNA{$mRNA_id};
            undef @exon_coord; undef @CDS_coord; $length=0; 
            foreach(@exon){ 
                if(/(chr_\d+|chr\w+)\s+\S+\s+exon\s+(\d+)\s+(\d+)\s+.*?transcript_id\s+"($mRNA_id)";/){
                    push(@exon_coord,($2,$3)); 
                    $length=$length+$3-$2+1;
                    push(@CDS_coord,($2,$3));
                }
            ...
            my @ORF_list = (); # 初始化数组
            foreach my $key (keys %ORF_list){ 
                if($ORF_list{$key} eq $mRNA_id){
                    push @ORF_list, $list2{$key}; 
                } 
            } 
            $n=@ORF_list; 
            print "$n\t";
            ...
            foreach my $x (@ORF_list){
                ...
                while($i<@exon_coord){ #print "i\t$i\n";
                    ...
                    if($i+1 < @exon_coord && $exon_coord[$i+1]>$start ){ 
                        ...
                        shift @CDS_coord if @CDS_coord;
                        shift @CDS_coord if @CDS_coord;
                        $L1=$L1-($exon_coord[$i+1]-$exon_coord[$i])-1 if $i+1 < @exon_coord;# print "m:\t@m\n CDS_corrd:\t@CDS_coord\n";
                    }
                    ...
                    if($i+1 < @exon_coord && $exon_coord[$i+1]>$end ){ 
                        if($L2>0){ 
                            ...
                            pop @CDS_coord if @CDS_coord;
                            pop @CDS_coord if @CDS_coord; #print "n:\t@n\n CDS_corrd:\t@CDS_coord\n";
                        }
